...
function SNSRegister() {
  ...
  const credentialData = location.state?.credentialData;

  const [email, setEmail] = useState(credentialData?.email || "");
  const [username, setName] = useState(credentialData?.name || "");
  const [password, setPassword] = useState("");
  const navigate = useNavigate();

  const handleSignUp = async () => {
    try {
      // Send a request to your backend to register the u er
